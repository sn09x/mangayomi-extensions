...
class DefaultExtension extends MProvider {
    ...
    async search(query, page, filters) {
        ...
        var sort = (filters[0].values[filters[0].state] ?? filters[0].values[0]).value;
        var order = (filters[1].values[filters[1].state] ?? filters[1].values[0]).value;
        var translation = (filters[2].values[filters[2].state] ?? filters[2].values[0]).value;
