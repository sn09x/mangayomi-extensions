...
class DefaultExtension extends MProvider {
    ...
    // 1. Initialize variables with default API values (leave as "" if the API handles empty params)
    var sort = ""; 
    var order = "";
    var translation = "";
    var status = "";

    // 2. Safely check if filters exist and are populated
    if (filters && filters.length >= 4) {
        // Optional Chaining (?.) is used here as an extra layer of safety
        sort = filters[0]?.values?.[filters[0]?.state]?.value || sort;
        order = filters[1]?.values?.[filters[1]?.state]?.value || order;
        translation = filters[2]?.values?.[filters[2]?.state]?.value || translation;
    ...
    // Continue with your fetch/API request here...
    // Example: let url = `https://weebcentral.com/search?query=${query}&offset=${offset}&sort=${sort}...`
        var type = "";

// Safely check if filters[4] and its state array exist before looping
if (filters?.[4]?.state) {
